...
class SyncService {
  ...
  final String _baseUrl = 'http://10.11.9.148:9223/paginaXApp';
  ...
  Future<void> syncProductos(String token) async {
    try {
      ...
      if (response.data is List) {
        List<Map<String, dynamic>> productos = List<Map<String, dynamic>>.from(response.data);
        ...
        if (response.data.containsKey('productos') && response.data['productos'] is List) {
          List<Map<String, dynamic>> productos = List<Map<String, dynamic>>.from(response.data['productos']);
          print('Número de productos recibidos: ${productos.length}');
          await _databaseHelper.upsertProductos(productos);
          print('Sincronización completada con éxito');
        } else {
          throw Exception('La respuesta no contiene una lista de productos válida');
        }
      } else {
